...
> 主要参考该[文章](https://jimmyswebnote.com/cloudflare-images-introduction/)
...
2. Worker 用应用程序密钥签名，访问 Backblaze B2 私密桶，
3. Backblaze B2 验证签名，返回图片给 Worker ，
4. Worker 显示图片到页面，用户看到图片。

### **配置 Backblaze B2 私密桶**

取个名字，别的保持默认。创建了之后记录Endpoint。私密桶的Lifecycle Settings > Keep only the last version of the file

创建应用程序密钥 (Application Key)

取个名儿，选择Read and Write，保持默认。

保存好keyID和applicationKey

### **创建 Cloudflare Worker**
Workers & Pages > create > Workers > Hello world 模板先初始化 > deploy 
...
Bucket Settings > Bucket Info > ```{"Cache-Control": "public, max-age=86400"} ``` 86400 表示缓存一天，可以设置更大。

## 测试

上传个图片，点击图片url: 此时可见直接访问 S3 URL是看不到图片的。<br>
然后用cf的域名就妥当了。
